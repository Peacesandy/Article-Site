...
Regular expressions can be used to find the parts of a sting that match a pattern. It can be used to search for a match in a particular piece of content.

They are also used to validate that a string follows a pattern. e.g., to verify that an email string is valid.

Regular expressions can also be used to verify if a password meets certain criteria.
...
There are four common flags in Regex.
